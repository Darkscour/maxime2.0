...
/**
 * Client-side filtering/sorting UI for the recruitment portal.
 *
 * The actual player list is fetched on the server (see ./page.tsx) and
 * passed in as a prop — this component just handles user interaction.
 * Keeping the data fetch on the server means:
 *   - No `useEffect` + loading spinners on first paint.
 *   - The Prisma client never ships to the browser.
 *   - SEO-friendly: the initial HTML already contains the players.
 */
...
import { type Player, RANKS } from "@/lib/mock-data";
import {
  FiltersPanel,
  DEFAULT_FILTERS,
...
export function RecruitmentPortal({ players }: { players: Player[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return players
      .filter((p) => {
        ...
        if (filters.games.length && !filters.games.includes(p.game))
          return false;
    ...
    <section className="relative overflow-hidden border-b border-white/5 bg-spotlight">
      ...
      <Container className="relative py-16 sm:py-20">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <Badge tone="zinc">
            Data sources: PandaScore · Riot · OpenDota · FACEIT
          </Badge>
        </div>
      </Container>
    </section>
